...
(function () {
  ...
  // On the Drop Locations page, keep only the WhatsApp bubble (no floating Call).
  var HIDE_CALL = /drop-locations/i.test(location.pathname);
  ...
  el.innerHTML =
    '<div id="ir-fab-panel" class="hidden w-72 max-w-[calc(100vw-2.5rem)] rounded-2xl bg-white shadow-2xl ring-1 ring-black/5 overflow-hidden">' +
      ...
      '</div>' +
      (HIDE_CALL ? '' : '<a href="tel:+' + PHONE + '" class="block bg-green-50 text-green-800 text-sm font-semibold text-center py-3 border-t border-green-100 hover:bg-green-100 transition-colors">Call +91 72020 35700</a>') +
    '</div>' +
    '<div class="flex items-center gap-3">' +
      (HIDE_CALL ? '' : '<a href="tel:+' + PHONE + '" aria-label="Call India Recycles" class="w-12 h-12 rounded-full bg-white shadow-lg ring-1 ring-black/5 text-green-700 flex items-center justify-center hover:bg-green-50 transition-colors">' + CALL + '</a>') +
